...
@section('title', 'Thêm Cây trồng mới')
@section('page-title', 'Thêm Cây trồng mới')
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-success text-white">
                    <h5 class="card-title mb-0">
                        <i class="fas fa-seedling me-2"></i>Thông tin Cây trồng mới
                    </h5>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('caytrong.store') }}">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('loai_cay') is-invalid @enderror" 
                                    ...
                                    <option value="lúa" {{ old('loai_cay') === 'lúa' ? 'selected' : '' }}>Lúa</option>
                                    <option value="ngô" {{ old('loai_cay') === 'ngô' ? 'selected' : '' }}>Ngô</option>
                                    <option value="khoai_lang" {{ old('loai_cay') === 'khoai_lang' ? 'selected' : '' }}>Khoai lang</option>
                                    <option value="đậu_phộng" {{ old('loai_cay') === 'đậu_phộng' ? 'selected' : '' }}>Đậu phộng</option>
                                    <option value="cà_chua" {{ old('loai_cay') === 'cà_chua' ? 'selected' : '' }}>Cà chua</option>
                                    <option value="rau_muống" {{ old('loai_cay') === 'rau_muống' ? 'selected' : '' }}>Rau muống</option>
                                    <option value="khác" {{ old('loai_cay') === 'khác' ? 'selected' : '' }}>Khác</option>
                                </select>
                                @error('loai_cay')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                ...
                                <select class="form-select @error('thua_dat_id') is-invalid @enderror" 
                                    ...
                                    @foreach($thuaDats as $thuaDat)
                                        <option value="{{ $thuaDat->id }}" 
                                                {{ old('thua_dat_id') == $thuaDat->id ? 'selected' : '' }}>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" 
                                       ...
                                       id="ngay_trong" 
                                       name="ngay_trong" 
                                       value="{{ old('ngay_trong', now()->format('Y-m-d')) }}">
                                @error('ngay_trong')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('trang_thai') is-invalid @enderror" 
                                        id="trang_thai" 
                                        name="trang_thai">
                                    <option value="đang_phát_triển" {{ old('trang_thai', 'đang_phát_triển') === 'đang_phát_triển' ? 'selected' : '' }}>
                                        Đang phát triển
                                    </option>
                                    <option value="thu_hoạch" {{ old('trang_thai') === 'thu_hoạch' ? 'selected' : '' }}>
                                        Thu hoạch
                                    </option>
                                    <option value="hoàn_thành" {{ old('trang_thai') === 'hoàn_thành' ? 'selected' : '' }}>
                                        Hoàn thành
                                    </option>
                                </select>
                                @error('trang_thai')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('ghi_chu') is-invalid @enderror" 
                                      ...
                                      name="ghi_chu" 
                                      rows="3"
                                      placeholder="Nhập ghi chú về cây trồng (tùy chọn)">{{ old('ghi_chu') }}</textarea>
                            @error('ghi_chu')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        thuaDatSelect.addEventListener('change', function() {
